...
public sealed class KafkaTopologyConfiguratorTests
{
    private static KafkaTopologyConfigurator CreateConfigurator() => new();
    ...
    public void DeclareQueue_WithArguments_IncludedInBuild()
    {
        // Arrange
        var sut = CreateConfigurator();
        Dictionary<string, object> arguments = new() { ["bw.kafka.partitions"] = 4 };

        // Act
        sut.DeclareQueue("orders", arguments: arguments);
        TopologyDeclaration result = sut.Build();

        // Assert
        result.Queues[0].Arguments.Should().NotBeNull();
        result.Queues[0].Arguments!["bw.kafka.partitions"].Should().Be(4);
    }
    ...
    public void DeclareQueue_WithFluentConfigure_ProducesCorrectArguments()
    {
        ...
        // Act
        sut.DeclareQueue("order-topic", durable: true, autoDelete: false, configure: q =>
        {
            q.Argument("bw.kafka.partitions", 4);
        });
        TopologyDeclaration result = sut.Build();

        // Assert
        result.Queues.Should().HaveCount(1);
        result.Queues[0].Name.Should().Be("order-topic");
    ...
    public void DeclareQueue_WithFluentConfigure_NullAction_ThrowsArgumentNullException()
    {
        ...
        // Act
        Action act = () => sut.DeclareQueue("test-topic", durable: true, autoDelete: false,
            configure: null!);
