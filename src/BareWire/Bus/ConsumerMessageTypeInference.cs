...
/// sugar overload <c>Consumer&lt;TConsumer&gt;()</c>, by inspecting the <see cref="IConsumer{T}"/>
/// interfaces the consumer implements. Runs <strong>once at startup</strong> (configuration time),
/// never per message — the dispatch hot path reads the materialized <c>ConsumerRegistration</c> and
/// performs no reflection (ADR-003 zero-copy / zero per-message reflection).
/// </summary>
