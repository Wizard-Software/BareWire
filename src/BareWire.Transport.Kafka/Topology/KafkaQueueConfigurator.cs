...
namespace BareWire.Transport.Kafka.Topology;

/// <summary>
/// Accumulates queue (topic) arguments via the fluent <see cref="IQueueConfigurator"/> API
/// and produces an immutable dictionary for <see cref="QueueDeclaration"/>.
/// In the Kafka transport the standard RabbitMQ-flavoured methods (dead-letter, TTL, etc.)
/// are accepted without error but have no native Kafka equivalent; use
/// <see cref="Argument"/> with <c>bw.kafka.*</c> keys (see <c>KafkaTopologyArguments</c>)
/// to configure Kafka-specific parameters such as partition count and retention.
/// </summary>
internal sealed class KafkaQueueConfigurator : IQueueConfigurator
{
