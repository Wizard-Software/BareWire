...
// A single instance is created by InMemoryConfigurator and SHARED BY REFERENCE with the
// lazily-created InMemoryTopologyConfigurator, so every write path — MapExchange<T> /
...
internal sealed class PublishRegistry
{
    ...
    // a DIFFERENT exchange or routing key from a second registration path) is recorded here. Zero
    // runtime cost: this list never leaves config time.
