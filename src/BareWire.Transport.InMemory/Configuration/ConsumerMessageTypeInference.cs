...
/// sugar overload <c>Consumer&lt;TConsumer&gt;()</c> on the in-memory transport endpoint configurator,
/// by inspecting the <see cref="IConsumer{T}"/> interfaces the consumer implements. Runs
/// <strong>once at startup</strong> (configuration time), never per message. Duplicated per project
/// (Core/Transport internals are not shared — package dependency rule: Transport.InMemory depends on
