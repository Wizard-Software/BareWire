...
internal sealed class InMemoryEndpointConfiguration : IReceiveEndpointConfigurator
{
    ...
    internal InMemoryEndpointConfiguration(string queueName)
    {
    ...
    /// <summary>
    /// The opt-in topology fragments declared by this endpoint's consumers. Empty when no consumer
    /// opted in (manual topology unchanged). Merged into <see cref="InMemoryTransportOptions.Topology"/>
    /// at build time.
    /// </summary>
    ...
    /// </summary>
    private static readonly MethodInfo TypedConsumerMethod = typeof(InMemoryEndpointConfiguration)
        ...
        where TConsumer : class
    {
        ...
        // the closed Consumer<TConsumer, TMessage>() delegate ONCE at startup and delegate to it — keeps
        ...
        where TMessage : class
    {
        ...
        InMemoryConsumerConfigurator<TConsumer, TMessage> configurator = new();
        configure(configurator);
        _consumerTypes.Add(typeof(TConsumer));
        _consumerRegistrations.Add(configurator.Build());

        // Opt-in topology is a SEPARATE, PARALLEL output from Build()/ConsumerRegistration: collect the
        // fragment only when it was declared (null == opt-in signal, manual topology unchanged).
    ...
    /// <summary>
    /// Minimal package-local carrier capturing per-key ordering settings declared on an in-memory
    /// endpoint. The in-memory package cannot reference the Core carrier (package dependency rule:
    /// Transport.InMemory depends on Abstractions only), so the storage is duplicated minimally here.
    /// </summary>
    /// <remarks>
