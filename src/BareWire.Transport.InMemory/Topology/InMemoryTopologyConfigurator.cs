...
/// </summary>
internal sealed class InMemoryTopologyConfigurator : ITopologyConfigurator
{
    ...
    // Shared BY REFERENCE with the owning InMemoryConfigurator so DeclareExchange<T> write-through
    // lands in the SAME per-type map set as MapExchange<T> / MapRoutingKey<T> / Publish<T> — one
    // source of truth, no merge step. A standalone instance gets a private registry: its
    // DeclareExchange<T> mappings are simply not consumed.
    private readonly PublishRegistry _publishRegistry;

    internal InMemoryTopologyConfigurator(PublishRegistry? publishRegistry = null) =>
        ...
        Action<IQueueConfigurator> configure)
    {
        ...
        var configurator = new InMemoryQueueConfigurator();
